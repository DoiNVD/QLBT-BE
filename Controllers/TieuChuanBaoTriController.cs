...
namespace NETCORE3.Controllers
{
    ...
    public class TieuChuanBaoTriController : ControllerBase
    {
        ...
        public ActionResult Get(string keyword)
        {
            ...
            var data = uow.tieuChuanBaoTris.GetAll(t => !t.IsDeleted).Select(x => new
            {
                x.Id,
                x.MaTieuChuan,
                x.TenTieuChuan,
                x.KyHieuBaoTri,
                ThoiGian = string.Format("{0:dd/MM/yyyy HH:mm:ss}", x.ThoiGian),
                x.QuyDinhTaiLieuHuongDan
            });
            if (data == null)
            {
                return NotFound();
