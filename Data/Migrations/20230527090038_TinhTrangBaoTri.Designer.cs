// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using NETCORE3.Data;

#nullable disable

namespace NETCORE3.Migrations
{
    [DbContext(typeof(MyDbContext))]
    [Migration("20230527090038_TinhTrangBaoTri")]
    partial class TinhTrangBaoTri
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "7.0.5")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<System.Guid>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("RoleId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<System.Guid>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<System.Guid>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderKey")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<System.Guid>", b =>
                {
                    b.Property<Guid>("UserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Value")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens", (string)null);
                });

            modelBuilder.Entity("NETCORE3.Data.MyDbContext+ApplicationRole", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("Name")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasDatabaseName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("AspNetRoles", (string)null);
                });

            modelBuilder.Entity("NETCORE3.Data.MyDbContext+ApplicationUser", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("int");

                    b.Property<Guid?>("BoPhan_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("ChucDanh")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid?>("ChucVu_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DonViTraLuong_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("DonVi_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Email")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("bit");

                    b.Property<string>("FullName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("MaNhanVien")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("MustChangePass")
                        .HasColumnType("bit");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("bit");

                    b.Property<Guid?>("Phongban_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("bit");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("UserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("BoPhan_Id")
                        .IsUnique()
                        .HasFilter("[BoPhan_Id] IS NOT NULL");

                    b.HasIndex("ChucVu_Id")
                        .IsUnique()
                        .HasFilter("[ChucVu_Id] IS NOT NULL");

                    b.HasIndex("DonViTraLuong_Id")
                        .IsUnique()
                        .HasFilter("[DonViTraLuong_Id] IS NOT NULL");

                    b.HasIndex("DonVi_Id")
                        .IsUnique()
                        .HasFilter("[DonVi_Id] IS NOT NULL");

                    b.HasIndex("NormalizedEmail")
                        .HasDatabaseName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasDatabaseName("UserNameIndex")
                        .HasFilter("[NormalizedUserName] IS NOT NULL");

                    b.HasIndex("Phongban_Id")
                        .IsUnique()
                        .HasFilter("[Phongban_Id] IS NOT NULL");

                    b.ToTable("AspNetUsers", (string)null);
                });

            modelBuilder.Entity("NETCORE3.Data.MyDbContext+ApplicationUserRole", b =>
                {
                    b.Property<Guid>("UserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("RoleId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles", (string)null);
                });

            modelBuilder.Entity("NETCORE3.Models.BoPhan", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("MaBoPhan")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<Guid?>("PhongBan_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("TenBoPhan")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("PhongBan_Id");

                    b.ToTable("BoPhans");
                });

            modelBuilder.Entity("NETCORE3.Models.ChiTietLoaiThongTinThietBi", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<Guid>("LoaiThietBi_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ThongTinThietBi_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("LoaiThietBi_Id");

                    b.HasIndex("ThongTinThietBi_Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("chiTietLoaiThongTinThietBis");
                });

            modelBuilder.Entity("NETCORE3.Models.ChucVu", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("BoPhan_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("MaChucVu")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TenChucVu")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("BoPhan_Id");

                    b.ToTable("chucVus");
                });

            modelBuilder.Entity("NETCORE3.Models.ChuyenNgayBaoTri", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("LyDoChuyen")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<string>("MaChuyenNgayBaoTri")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("NgayChuyen")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("ChuyeNgayBaoTris");
                });

            modelBuilder.Entity("NETCORE3.Models.DanhMucKho", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("MaKho")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TenKho")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("User_Id")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("User_CreatedId");

                    b.HasIndex("User_Id");

                    b.ToTable("danhMucKhos");
                });

            modelBuilder.Entity("NETCORE3.Models.Domain", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("MaDomain")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TenDomain")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("domains");
                });

            modelBuilder.Entity("NETCORE3.Models.DonVi", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("MaDonVi")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<Guid?>("TapDoan_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("TenDonVi")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("TapDoan_Id");

                    b.ToTable("DonVis");
                });

            modelBuilder.Entity("NETCORE3.Models.DonViTinh", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("MaDonViTinh")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TenDonViTinh")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("DonViTinhs");
                });

            modelBuilder.Entity("NETCORE3.Models.DonViTraLuong", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("MaDonViTraLuong")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TenDonViTraLuong")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.ToTable("donViTraLuongs");
                });

            modelBuilder.Entity("NETCORE3.Models.HangThietBi", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("HeThong_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("MaHangThietBi")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TenHang")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("HeThong_Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("hangThietBis");
                });

            modelBuilder.Entity("NETCORE3.Models.HeThong", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("MaHeThong")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TenHeThong")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("heThongs");
                });

            modelBuilder.Entity("NETCORE3.Models.HuyBaoTri", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("LyDoHuy")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<string>("MaHuy")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("HuyBaoTris");
                });

            modelBuilder.Entity("NETCORE3.Models.Kho", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DanhMucKho_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DonViTinh_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("DonVi_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<int>("SoLuong")
                        .HasColumnType("int");

                    b.Property<string>("TinhTrangThietBi")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("UserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("User_Id")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("DanhMucKho_Id");

                    b.HasIndex("DonViTinh_Id");

                    b.HasIndex("DonVi_Id");

                    b.HasIndex("UserId");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("khos");
                });

            modelBuilder.Entity("NETCORE3.Models.KhoLoaiThietBi", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<Guid>("Kho_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("LoaiThietBi_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("Kho_Id");

                    b.HasIndex("LoaiThietBi_Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("khoLoaiThietBis");
                });

            modelBuilder.Entity("NETCORE3.Models.KhoThongTinThietBi", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<Guid>("Kho_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ThongTinThietBi_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("Kho_Id");

                    b.HasIndex("ThongTinThietBi_Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("khoThongTinThietBis");
                });

            modelBuilder.Entity("NETCORE3.Models.LoaiHangThietBi", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("HangThietBi_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<Guid>("LoaiThietBi_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("HangThietBi_Id");

                    b.HasIndex("LoaiThietBi_Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("loaiHangThietBis");
                });

            modelBuilder.Entity("NETCORE3.Models.LoaiThietBi", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("MaLoaiThietBi")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TenLoaiThietBi")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("loaiThietBis");
                });

            modelBuilder.Entity("NETCORE3.Models.LoaiThongTinThietBi", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<Guid>("LoaiThietBi_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ThongTinThietBi_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("LoaiThietBi_Id");

                    b.HasIndex("ThongTinThietBi_Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("loaiThongTinThietBis");
                });

            modelBuilder.Entity("NETCORE3.Models.Log", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("AccessDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("AccessdBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Data")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("IpAddress")
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<string>("Type")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("Url")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.HasKey("Id");

                    b.HasIndex("AccessdBy");

                    b.ToTable("Logs");
                });

            modelBuilder.Entity("NETCORE3.Models.Loi", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("MaLoi")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TenLoi")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("lois");
                });

            modelBuilder.Entity("NETCORE3.Models.LoiThietBiSuaChua", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<Guid>("Loi_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ThietBiSuaChua_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("Loi_Id");

                    b.HasIndex("ThietBiSuaChua_Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("loithietbisuachuas");
                });

            modelBuilder.Entity("NETCORE3.Models.Menu", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Icon")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<Guid?>("Parent_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("TenMenu")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<int>("ThuTu")
                        .HasColumnType("int");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Url")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("Menus");
                });

            modelBuilder.Entity("NETCORE3.Models.Menu_Role", b =>
                {
                    b.Property<Guid>("Menu_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("Role_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Add")
                        .HasColumnType("bit");

                    b.Property<bool>("Cof")
                        .HasColumnType("bit");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("Del")
                        .HasColumnType("bit");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("Edit")
                        .HasColumnType("bit");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<bool>("Print")
                        .HasColumnType("bit");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("View")
                        .HasColumnType("bit");

                    b.HasKey("Menu_Id", "Role_Id");

                    b.HasIndex("Role_Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("Menu_Roles");
                });

            modelBuilder.Entity("NETCORE3.Models.NhaCungCap", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("DiaChi")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Email")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("MaNhaCungCap")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("NguoiLienHe")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("SoDienThoai")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TenNhaCungCap")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("NhaCungCaps");
                });

            modelBuilder.Entity("NETCORE3.Models.Nhom", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("MaNhom")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TenNhom")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("Nhoms");
                });

            modelBuilder.Entity("NETCORE3.Models.PhanHoi", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("Date")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("GopY")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Image")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<Guid>("NguoiDung_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("NoiDungPhanHoi")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("TrangThaiXuLy")
                        .HasColumnType("int");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("PhanHois");
                });

            modelBuilder.Entity("NETCORE3.Models.Phongban", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DonVi_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("MaPhongBan")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TenPhongBan")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("DonVi_Id");

                    b.ToTable("Phongbans");
                });

            modelBuilder.Entity("NETCORE3.Models.PhuongThucDangNhap", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Domain")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("LinkCheckLogin")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TenPhuongThucDangNhap")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("PhuongThucDangNhaps");
                });

            modelBuilder.Entity("NETCORE3.Models.TapDoan", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("MaTapDoan")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TenTapDoan")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("tapDoans");
                });

            modelBuilder.Entity("NETCORE3.Models.ThietBiSuaChua", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DonViTinh_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("GhiChu")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("MaThietBiSuaChua")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("SoLuong")
                        .HasColumnType("int");

                    b.Property<Guid?>("ThongTinThietBi_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("DonViTinh_Id");

                    b.HasIndex("ThongTinThietBi_Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("thietbisuachuas");
                });

            modelBuilder.Entity("NETCORE3.Models.ThongTinHangThietBi", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("HangThietBi_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<Guid>("ThongTinThietBi_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("HangThietBi_Id");

                    b.HasIndex("ThongTinThietBi_Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("thongTinHangThietBis");
                });

            modelBuilder.Entity("NETCORE3.Models.ThongTinThietBi", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("CauHinh")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("Domain_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("HangThietBi_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("MaThongTinThietBi")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("ModelThietBi")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid?>("NhaCungCap_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("SoSeri")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TenThietBi")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<DateTime>("ThoiGianBaoHanh")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("Domain_Id");

                    b.HasIndex("HangThietBi_Id");

                    b.HasIndex("NhaCungCap_Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("thongTinThietBis");
                });

            modelBuilder.Entity("NETCORE3.Models.ThucHienBaoTri", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("DanhGiaNguoiDung")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("GhiChu")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("HinhAnh")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("MaThucHienBaoTri")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<Guid?>("ThietBiSuaChua_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("ThoiGianBatDauBaoTri")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("ThoiGianKetThuc")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("TieuChuanBaoTri_Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("ThietBiSuaChua_Id");

                    b.HasIndex("TieuChuanBaoTri_Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("thucHienBaoTris");
                });

            modelBuilder.Entity("NETCORE3.Models.TieuChuanBaoTri", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("KyHieuBaoTri")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("MaTieuChuan")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("QuyDinhTaiLieuHuongDan")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TenTieuChuan")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<DateTime>("ThoiGian")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("tieuChuanBaoTris");
                });

            modelBuilder.Entity("NETCORE3.Models.TinhTrangBaoTri", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DeletedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeletedDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("MaTinhTrangBaoTri")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TenTinhTrangBaoTri")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<Guid?>("UpdatedBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("User_CreatedId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("User_CreatedId");

                    b.ToTable("tinhTrangBaoTris");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<System.Guid>", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<System.Guid>", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<System.Guid>", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<System.Guid>", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("NETCORE3.Data.MyDbContext+ApplicationUser", b =>
                {
                    b.HasOne("NETCORE3.Models.BoPhan", "BoPhan")
                        .WithOne("User_Created")
                        .HasForeignKey("NETCORE3.Data.MyDbContext+ApplicationUser", "BoPhan_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("NETCORE3.Models.ChucVu", "ChucVu")
                        .WithOne("User_Created")
                        .HasForeignKey("NETCORE3.Data.MyDbContext+ApplicationUser", "ChucVu_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("NETCORE3.Models.DonViTraLuong", "DonViTraLuong")
                        .WithOne("User_Created")
                        .HasForeignKey("NETCORE3.Data.MyDbContext+ApplicationUser", "DonViTraLuong_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("NETCORE3.Models.DonVi", "DonVi")
                        .WithOne("User_Created")
                        .HasForeignKey("NETCORE3.Data.MyDbContext+ApplicationUser", "DonVi_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("NETCORE3.Models.Phongban", "Phongban")
                        .WithOne("User_Created")
                        .HasForeignKey("NETCORE3.Data.MyDbContext+ApplicationUser", "Phongban_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("BoPhan");

                    b.Navigation("ChucVu");

                    b.Navigation("DonVi");

                    b.Navigation("DonViTraLuong");

                    b.Navigation("Phongban");
                });

            modelBuilder.Entity("NETCORE3.Data.MyDbContext+ApplicationUserRole", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationRole", "Role")
                        .WithMany("UserRoles")
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User")
                        .WithMany("UserRoles")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Role");

                    b.Navigation("User");
                });

            modelBuilder.Entity("NETCORE3.Models.BoPhan", b =>
                {
                    b.HasOne("NETCORE3.Models.Phongban", "Phongban")
                        .WithMany()
                        .HasForeignKey("PhongBan_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Phongban");
                });

            modelBuilder.Entity("NETCORE3.Models.ChiTietLoaiThongTinThietBi", b =>
                {
                    b.HasOne("NETCORE3.Models.LoaiThietBi", "LoaiThietBi")
                        .WithMany()
                        .HasForeignKey("LoaiThietBi_Id")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("NETCORE3.Models.ThongTinThietBi", "ThongTinThietBi")
                        .WithMany("ChiTietLoaiThongTinThietBis")
                        .HasForeignKey("ThongTinThietBi_Id")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("LoaiThietBi");

                    b.Navigation("ThongTinThietBi");

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.ChucVu", b =>
                {
                    b.HasOne("NETCORE3.Models.BoPhan", "BoPhan")
                        .WithMany()
                        .HasForeignKey("BoPhan_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("BoPhan");
                });

            modelBuilder.Entity("NETCORE3.Models.ChuyenNgayBaoTri", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.DanhMucKho", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User")
                        .WithMany()
                        .HasForeignKey("User_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("User");

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.Domain", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.DonVi", b =>
                {
                    b.HasOne("NETCORE3.Models.TapDoan", "TapDoan")
                        .WithMany()
                        .HasForeignKey("TapDoan_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("TapDoan");
                });

            modelBuilder.Entity("NETCORE3.Models.DonViTinh", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.HangThietBi", b =>
                {
                    b.HasOne("NETCORE3.Models.HeThong", "HeThong")
                        .WithMany()
                        .HasForeignKey("HeThong_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("HeThong");

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.HeThong", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.HuyBaoTri", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.Kho", b =>
                {
                    b.HasOne("NETCORE3.Models.DanhMucKho", "DanhMucKho")
                        .WithMany()
                        .HasForeignKey("DanhMucKho_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("NETCORE3.Models.DonViTinh", "DonViTinh")
                        .WithMany()
                        .HasForeignKey("DonViTinh_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("NETCORE3.Models.DonVi", "DonVi")
                        .WithMany()
                        .HasForeignKey("DonVi_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("DanhMucKho");

                    b.Navigation("DonVi");

                    b.Navigation("DonViTinh");

                    b.Navigation("User");

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.KhoLoaiThietBi", b =>
                {
                    b.HasOne("NETCORE3.Models.Kho", "Kho")
                        .WithMany("khoLoaiThietBis")
                        .HasForeignKey("Kho_Id")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("NETCORE3.Models.LoaiThietBi", "LoaiThietBi")
                        .WithMany()
                        .HasForeignKey("LoaiThietBi_Id")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Kho");

                    b.Navigation("LoaiThietBi");

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.KhoThongTinThietBi", b =>
                {
                    b.HasOne("NETCORE3.Models.Kho", "Kho")
                        .WithMany("khoThongTinThietBis")
                        .HasForeignKey("Kho_Id")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("NETCORE3.Models.ThongTinThietBi", "ThongTinThietBi")
                        .WithMany()
                        .HasForeignKey("ThongTinThietBi_Id")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Kho");

                    b.Navigation("ThongTinThietBi");

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.LoaiHangThietBi", b =>
                {
                    b.HasOne("NETCORE3.Models.HangThietBi", "HangThietBi")
                        .WithMany("loaiHangThietBis")
                        .HasForeignKey("HangThietBi_Id")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("NETCORE3.Models.LoaiThietBi", "LoaiThietBi")
                        .WithMany()
                        .HasForeignKey("LoaiThietBi_Id")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("HangThietBi");

                    b.Navigation("LoaiThietBi");

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.LoaiThietBi", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.LoaiThongTinThietBi", b =>
                {
                    b.HasOne("NETCORE3.Models.LoaiThietBi", "LoaiThietBi")
                        .WithMany()
                        .HasForeignKey("LoaiThietBi_Id")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("NETCORE3.Models.ThongTinThietBi", "ThongTinThietBi")
                        .WithMany()
                        .HasForeignKey("ThongTinThietBi_Id")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("LoaiThietBi");

                    b.Navigation("ThongTinThietBi");

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.Log", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "ApplicationUser")
                        .WithMany()
                        .HasForeignKey("AccessdBy")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("ApplicationUser");
                });

            modelBuilder.Entity("NETCORE3.Models.Loi", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.LoiThietBiSuaChua", b =>
                {
                    b.HasOne("NETCORE3.Models.Loi", "Loi")
                        .WithMany()
                        .HasForeignKey("Loi_Id")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("NETCORE3.Models.ThietBiSuaChua", "ThietBiSuaChua")
                        .WithMany("loiThietBiSuaChuas")
                        .HasForeignKey("ThietBiSuaChua_Id")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Loi");

                    b.Navigation("ThietBiSuaChua");

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.Menu", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.Menu_Role", b =>
                {
                    b.HasOne("NETCORE3.Models.Menu", "Menu")
                        .WithMany("Menu_Roles")
                        .HasForeignKey("Menu_Id")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationRole", "Role")
                        .WithMany("Menu_Roles")
                        .HasForeignKey("Role_Id")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Menu");

                    b.Navigation("Role");

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.NhaCungCap", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.Nhom", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.PhanHoi", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.Phongban", b =>
                {
                    b.HasOne("NETCORE3.Models.DonVi", "DonVi")
                        .WithMany()
                        .HasForeignKey("DonVi_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("DonVi");
                });

            modelBuilder.Entity("NETCORE3.Models.PhuongThucDangNhap", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.TapDoan", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.ThietBiSuaChua", b =>
                {
                    b.HasOne("NETCORE3.Models.DonViTinh", "DonViTinh")
                        .WithMany()
                        .HasForeignKey("DonViTinh_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("NETCORE3.Models.ThongTinThietBi", "ThongTinThietBi")
                        .WithMany()
                        .HasForeignKey("ThongTinThietBi_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("DonViTinh");

                    b.Navigation("ThongTinThietBi");

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.ThongTinHangThietBi", b =>
                {
                    b.HasOne("NETCORE3.Models.HangThietBi", "HangThietBi")
                        .WithMany()
                        .HasForeignKey("HangThietBi_Id")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("NETCORE3.Models.ThongTinThietBi", "ThongTinThietBi")
                        .WithMany()
                        .HasForeignKey("ThongTinThietBi_Id")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("HangThietBi");

                    b.Navigation("ThongTinThietBi");

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.ThongTinThietBi", b =>
                {
                    b.HasOne("NETCORE3.Models.Domain", "Domain")
                        .WithMany()
                        .HasForeignKey("Domain_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("NETCORE3.Models.HangThietBi", "HangThietBi")
                        .WithMany()
                        .HasForeignKey("HangThietBi_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("NETCORE3.Models.NhaCungCap", "NhaCungCap")
                        .WithMany()
                        .HasForeignKey("NhaCungCap_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Domain");

                    b.Navigation("HangThietBi");

                    b.Navigation("NhaCungCap");

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.ThucHienBaoTri", b =>
                {
                    b.HasOne("NETCORE3.Models.ThietBiSuaChua", "ThietBiSuaChua")
                        .WithMany()
                        .HasForeignKey("ThietBiSuaChua_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("NETCORE3.Models.TieuChuanBaoTri", "TieuChuanBaoTri")
                        .WithMany()
                        .HasForeignKey("TieuChuanBaoTri_Id")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("ThietBiSuaChua");

                    b.Navigation("TieuChuanBaoTri");

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.TieuChuanBaoTri", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.TinhTrangBaoTri", b =>
                {
                    b.HasOne("NETCORE3.Data.MyDbContext+ApplicationUser", "User_Created")
                        .WithMany()
                        .HasForeignKey("User_CreatedId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Data.MyDbContext+ApplicationRole", b =>
                {
                    b.Navigation("Menu_Roles");

                    b.Navigation("UserRoles");
                });

            modelBuilder.Entity("NETCORE3.Data.MyDbContext+ApplicationUser", b =>
                {
                    b.Navigation("UserRoles");
                });

            modelBuilder.Entity("NETCORE3.Models.BoPhan", b =>
                {
                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.ChucVu", b =>
                {
                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.DonVi", b =>
                {
                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.DonViTraLuong", b =>
                {
                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.HangThietBi", b =>
                {
                    b.Navigation("loaiHangThietBis");
                });

            modelBuilder.Entity("NETCORE3.Models.Kho", b =>
                {
                    b.Navigation("khoLoaiThietBis");

                    b.Navigation("khoThongTinThietBis");
                });

            modelBuilder.Entity("NETCORE3.Models.Menu", b =>
                {
                    b.Navigation("Menu_Roles");
                });

            modelBuilder.Entity("NETCORE3.Models.Phongban", b =>
                {
                    b.Navigation("User_Created");
                });

            modelBuilder.Entity("NETCORE3.Models.ThietBiSuaChua", b =>
                {
                    b.Navigation("loiThietBiSuaChuas");
                });

            modelBuilder.Entity("NETCORE3.Models.ThongTinThietBi", b =>
                {
                    b.Navigation("ChiTietLoaiThongTinThietBis");
                });
#pragma warning restore 612, 618
        }
    }
}
